...
const LEGEND = [
  { cls: 'bg-jade/80', label: 'Booked' },
  { cls: 'bg-gold/20 border border-gold/50', label: 'Pending approval' },
  { cls: 'bg-surface-alt', label: 'Blocked' },
  { cls: 'bg-surface border border-rule', label: 'Available' },
  { cls: 'bg-jade-soft border border-jade/30', label: 'Selected' },
];
...
export default function CalendarPage() {
  ...
  // Mobile defaults to first villa (single-villa view); desktop can show all
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Booking approved.'); qc.invalidateQueries({ queryKey: ['host-bookings'] }); clearSelection(); },
    onError: () => toast.error('Could not approve booking.'),
  });
  const declineMutation = useMutation({
    mutationFn: (id) => api.post(`/host/bookings/${id}/decline/`, {}),
    onSuccess: () => { toast.success('Booking declined.'); qc.invalidateQueries({ queryKey: ['host-bookings'] }); clearSelection(); },
  ...
  const shownVillas = villaFilter === 'all'
    ...
    ? allVillas.slice(0, 1)   // single villa on initial load; user can change
  ...
  function handleSavePrice() {
    ...
    toast.success(`Price saved: Rp ${fmt} / night`);
    clearSelection();
  }
    ...
    <div className="flex flex-col gap-6 max-w-[1300px]">
      ...
      <div>
        <h1 className="font-display text-3xl font-medium text-ink">
          ...
          <span className="font-sans font-normal text-xl text-ink-mute">可用性与价格</span>
        </h1>
        <p className="text-sm text-ink-mute mt-1">
          Click a date to set price or block it. Drag to select a range.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          <option value="all">All villas ({allVillas.length})</option>
          {allVillas.map((v) => (
        ...
        <div className="flex items-center gap-1">
          <button
            type="button"
            onClick={prevMonth}
            className="size-9 flex items-center justify-center rounded-lg border border-rule bg-surface hover:bg-jade-soft hover:border-jade/30 transition-colors"
          ...
          {!isCurrentPeriod && (
            <button
              type="button"
              onClick={goToday}
              className="ml-1 px-3 h-9 text-xs font-semibold border border-rule rounded-lg bg-surface text-ink-soft hover:bg-jade-soft hover:border-jade/30 transition-colors"
            >
              Today
            </button>
          )}
      ...
      <div className="flex gap-5 items-start">
        ...
        {/* Calendar grid — cross-fade on month change */}
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-2 text-[11px] text-ink-mute">
            ...
            Showing {shownVillas.length} propert{shownVillas.length === 1 ? 'y' : 'ies'} · {new Date(year, month + 1, 0).getDate()} days
          </p>
        </div>
          ...
          <div className="w-72 shrink-0 bg-surface rounded-xl border border-rule shadow-sm overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-rule">
              <h3 className="text-sm font-semibold text-ink leading-tight">
                {panel.kind === 'booked'     ? 'Booking details' :
                 panel.kind === 'pending'    ? 'Pending approval' :
                 panel.kind === 'range'      ? `${panel.hi - panel.lo + 1} days selected` :
                                               'Date details'}
              </h3>
              <button
                type="button"
                onClick={clearSelection}
                className="size-6 flex items-center justify-center rounded-md text-ink-mute hover:bg-surface-alt transition-colors"
              >
            ...
            <div className="p-4 flex flex-col gap-4">
              ...
              {/* ── BOOKED / PENDING panel ── */}
              {(panel.kind === 'booked' || panel.kind === 'pending') && panel.bk && (
                <>
                  <div className="flex items-center gap-2.5">
                    <img
                      src={panel.bk.guestAvatarUrl}
                      alt={panel.bk.guestName}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-surface-alt rounded-lg p-2.5">
                      <p className="text-[10px] text-ink-mute font-semibold uppercase tracking-wide">Check-in</p>
                      <p className="text-xs font-semibold text-ink mt-1">{panel.bk.checkIn}</p>
                    </div>
                    <div className="bg-surface-alt rounded-lg p-2.5">
                      <p className="text-[10px] text-ink-mute font-semibold uppercase tracking-wide">Check-out</p>
                  ...
                  <div className="flex flex-col gap-1.5 text-xs">
                    ...
                    <div className="flex justify-between">
                      <span className="text-ink-mute">Your payout</span>
                  ...
                  {panel.kind === 'pending' && (
                    <div className="flex gap-2 pt-1">
                      <button
                        type="button"
                        onClick={() => handleApprove(panel.bk.id)}
                        disabled={approveMutation.isPending}
                        className="flex-1 h-8 text-xs font-semibold bg-jade text-white rounded-lg hover:bg-jade-deep disabled:opacity-50 transition-colors"
                      >
                        Approve
                      </button>
                      <button
                        type="button"
                        onClick={() => handleDecline(panel.bk.id)}
                        disabled={declineMutation.isPending}
                        className="flex-1 h-8 text-xs font-semibold border border-rule text-ink-soft rounded-lg hover:border-danger hover:text-danger disabled:opacity-50 transition-colors"
                      >
                        Decline
                      </button>
                    </div>
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-3 h-8 text-xs font-semibold bg-jade text-white rounded-lg hover:bg-jade-deep transition-colors shrink-0"
                      >
                        Save
                      </button>
                    </div>
                  ...
                  <div className="border-t border-rule pt-3">
                    <button
                      type="button"
                      onClick={handleBlock}
                      className="w-full h-8 text-xs font-semibold border border-rule text-ink-soft rounded-lg hover:border-danger hover:text-danger transition-colors flex items-center justify-center gap-1.5"
                    >
                      <Lock className="size-3" />
                      {panel.kind === 'range' ? 'Block these dates' : 'Block this date'}
                    </button>
                  </div>
                </>
              )}
      ...
      <div className="bg-surface rounded-xl border border-rule shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-rule flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-ink">Seasonal pricing rules</h2>
            <p className="text-xs text-ink-mute mt-0.5">
              Markups applied automatically across all 3 properties during peak periods
            </p>
          </div>
          <button
            type="button"
        ...
        {rules.length === 0 ? (
          <div className="py-10 text-center text-sm text-ink-mute">No rules yet.</div>
        ) : (
              ...
              <div key={rule.id} className="flex items-center gap-4 px-5 py-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    type="button"
                    onClick={() => toast.info('Rule editor coming soon')}
                    className="size-7 flex items-center justify-center rounded-md text-ink-mute hover:bg-jade-soft hover:text-jade transition-colors"
                    title="Edit rule"
                  >
                  ...
                  <button
                    ...
                    className="size-7 flex items-center justify-center rounded-md text-ink-mute hover:bg-danger/10 hover:text-danger transition-colors"
                    title="Remove rule"
                  >
