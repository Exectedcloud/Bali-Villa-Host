...
function MessageBubble({ msg, guestAvatar }) {
  ...
  const primaryText = isHost ? (msg.textEn || msg.textZh) : (msg.textZh || msg.textEn);
  const secondaryText = isHost ? (msg.textZh || null) : (msg.textEn || null);
  ...
  return (
    <div className={`flex gap-2.5 group ${isHost ? 'flex-row-reverse' : 'flex-row'}`}>
      {!isHost && (
        <Avatar src={guestAvatar} name="" size={28} className="self-end mb-0.5" />
      )}
      <div className={`flex flex-col gap-1 max-w-[72%] sm:max-w-[68%] ${isHost ? 'items-end' : 'items-start'}`}>
        <div className={`rounded-2xl px-3.5 py-2.5 space-y-1.5 ${isHost ? 'bg-jade text-white rounded-tr-sm' : 'bg-surface border border-rule text-ink rounded-tl-sm'}`}>
          <p className="text-sm leading-relaxed">{primaryText}</p>

          {secondaryText && (
            <>
              <div className={`h-px ${isHost ? 'bg-white/20' : 'bg-rule'}`} />
              <p className={`text-xs leading-relaxed ${isHost ? 'text-white/65' : 'text-mist-deep'}`}>
                {secondaryText}
              </p>
            </>
          )}
          ...
          <div className={`flex items-center gap-1.5 pt-0.5 ${isHost ? 'justify-end' : 'justify-start'}`}>
            ...
            {msg.translated && (
              <span
                title={confidence != null ? `DeepL · confidence ${Math.round(confidence * 100)}%` : 'Translated by DeepL'}
        ...
        {msg.translated && !isHost && (
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity pl-1">
            <button type="button" onClick={() => toast.success(t('feedbackGood'))} className="size-5 flex items-center justify-center rounded text-ink-mute hover:text-success transition-colors">
              <ThumbsUp className="size-3" />
            </button>
            <button type="button" onClick={() => toast.info(t('feedbackBad'))} className="size-5 flex items-center justify-center rounded text-ink-mute hover:text-danger transition-colors">
...
function Thread({ conv }) {
  ...
  const QUICK_REPLIES = [
    { key: 'checkIn',    body: QUICK_REPLY_BODIES.checkIn },
    { key: 'directions', body: QUICK_REPLY_BODIES.directions },
    { key: 'wifi',       body: QUICK_REPLY_BODIES.wifi },
    { key: 'houseRules', body: QUICK_REPLY_BODIES.houseRules },
  ];
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3">
        {messages.map((msg) => (
          <MessageBubble key={msg.id} msg={msg} guestAvatar={conv.guestAvatar} />
        ))}
        <div ref={endRef} />
      </div>
      ...
      <div className="border-t border-rule shrink-0 bg-surface">
            ...
            {showQR && (
              <div className="absolute bottom-full left-0 mb-2 w-72 bg-surface rounded-xl border border-rule shadow-lg overflow-hidden z-10">
                {QUICK_REPLIES.map((qr) => (
                  <button key={qr.key} type="button" onClick={() => { setMsgInput(qr.body); setShowQR(false); }} className="w-full text-left px-3.5 py-2.5 hover:bg-jade-soft transition-colors border-b border-rule last:border-0">
        ...
        <div className="flex items-end gap-2 px-4 pt-2 pb-4">
          <button type="button" onClick={() => toast.info(t('attachmentSoon'))} className="size-9 flex items-center justify-center rounded-lg border border-rule text-ink-mute hover:bg-jade-soft hover:border-jade/30 hover:text-jade transition-colors shrink-0">
    ...
    <div className="flex bg-surface rounded-xl border border-rule shadow-sm overflow-hidden -m-4 lg:-m-8" style={{ height: 'calc(100vh - 5.5rem)' }}>
      ...
      <div className={`border-r border-rule flex flex-col shrink-0 ${
        ...
        <div className="px-4 py-3.5 border-b border-rule shrink-0">
          <h2 className="text-sm font-semibold text-ink mb-2.5">{t('title')}</h2>
          <div className="relative">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 size-3.5 text-ink-mute pointer-events-none" />
            <input type="text" placeholder={t('searchPlaceholder')} value={search} onChange={(e) => setSearch(e.target.value)} className="w-full h-8 pl-8 pr-3 text-xs border border-rule rounded-lg bg-surface-alt focus:outline-none focus:ring-1 focus:ring-jade placeholder:text-ink-mute/60" />
          </div>
        </div>
            ...
            <button key={c.id} type="button" onClick={() => setActiveId(c.id)} className={`w-full text-left px-4 py-3.5 border-b border-rule last:border-0 transition-colors ${c.id === (activeConv?.id) ? 'bg-jade-soft border-l-4 border-l-jade' : 'hover:bg-surface-alt/60 border-l-4 border-l-transparent'}`}>
              <div className="flex items-start gap-2.5">
                <div className="relative shrink-0">
                  <Avatar src={c.guestAvatar} name={c.guestName} size={36} />
                  {c.unreadHost > 0 && (
