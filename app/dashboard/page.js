...
export default function DashboardPage() {
  ...
  function contextGreeting() {
    ...
    if (!firstName) return t('greeting.welcome');
    if (bookings.length === 0) return t('greeting.firstVilla', { name: firstName });
    if (checkInsThisWeek.length > 0) return t('greeting.checkIns', { name: firstName, count: checkInsThisWeek.length });
    return t('greeting.fillDates', { name: firstName });
  }

  const METRICS = [
    { label: t('metric.bookingsThisMonth'), value: String(activeThisMonth.length), icon: Calendar, trend: 'neutral', change: null },
    {
      label: t('metric.payoutThisMonth'),
    ...
    },
    {
      ...
      sub:   host?.totalBookings ? t('metric.fromBookings', { count: host.totalBookings }) : null,
      icon: Star, trend: 'neutral', isRating: true, change: null,
    },
    { label: t('metric.totalBookings'), value: host?.totalBookings != null ? String(host.totalBookings) : '—', icon: BarChart3, trend: 'neutral', change: null },
  ];

  const actionItems = [
    pendingCount > 0 && { id: 1, text: t('alert.pendingBookings', { count: pendingCount }), href: '/dashboard/reservations', action: t('alert.review'), primary: true },
    unreadCount  > 0 && { id: 2, text: t('alert.unreadMessages', { count: unreadCount }),  href: '/dashboard/messages',      action: t('alert.reply'),  primary: true },
    { id: 3, text: t('alert.updateCalendar'), href: '/dashboard/calendar', action: tCommon('edit'), primary: false },
  ].filter(Boolean);
    ...
    <div className="flex flex-col gap-8 max-w-6xl">
        ...
        <div className="flex flex-wrap gap-3">
          {pendingCount > 0 && (
            <Link href="/dashboard/reservations" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-warn/10 border border-warn/30 text-sm font-medium text-warn hover:bg-warn/15 transition-colors">
              ...
              {t('alert.pendingBookings', { count: pendingCount })}
          ...
          {unreadCount > 0 && (
            <Link href="/dashboard/messages" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-jade-soft border border-jade/30 text-sm font-medium text-jade hover:bg-jade/10 transition-colors">
              ...
              {t('alert.unreadMessages', { count: unreadCount })}
              <ArrowRight className="size-3.5" />
            </Link>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="flex flex-col gap-4">
          {[
            { title: t('todayCheckIns'),  guests: todayCheckIns,  badge: 'bg-success/10 text-success' },
            { title: t('todayCheckOuts'), guests: todayCheckOuts, badge: 'bg-jade-soft text-jade' },
            ...
            <div key={title} className="bg-surface rounded-xl border border-rule shadow-sm">
              ...
              <div className="px-5">
                {guests.length === 0 && <p className="text-xs text-ink-mute py-4 text-center">{t('noneToday')}</p>}
                {guests.map((b) => (
                  <div key={b.id} className="flex items-center gap-3 py-3 border-b border-rule last:border-0">
                    <Avatar src={b.guestAvatarUrl} name={b.guestName} size={36} />
                    ...
                    <span className="text-xs text-ink-mute shrink-0">{tCommon('nights', { count: b.nights })}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="bg-surface rounded-xl border border-rule shadow-sm">
          <div className="px-5 py-4 border-b border-rule">
            <h2 className="text-sm font-semibold text-ink">{t('actionItems')}</h2>
            <p className="text-xs text-ink-mute mt-0.5">{t('actionItemsDesc')}</p>
          </div>
          <div className="divide-y divide-rule">
            {actionItems.length === 0 && <p className="text-xs text-ink-mute p-5 text-center">{t('allCaughtUp')}</p>}
      ...
      <div className="bg-surface rounded-xl border border-rule shadow-sm">
        <div className="px-5 py-4 border-b border-rule flex items-center justify-between">
          <h2 className="text-sm font-semibold text-ink">{t('recentBookings')}</h2>
          <Link href="/dashboard/reservations" className="text-xs text-jade hover:text-jade-deep font-medium transition-colors">{tCommon('viewAll')}</Link>
        </div>
        <div className="divide-y divide-rule">
          {bookings.length === 0 && <p className="text-xs text-ink-mute p-5 text-center">{t('noBookingsYet')}</p>}
          {bookings.slice(0, 6).map((b) => {
            const statusKey = b.status === 'in_house' ? 'inHouse' : b.status === 'pending_approval' ? 'pendingApproval' : b.status;
            return (
              <div key={b.id} className="flex items-center gap-3 px-5 py-4">
                <Avatar src={b.guestAvatarUrl} name={b.guestName} size={32} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-ink truncate">{b.guestName} · {b.villa?.titleEn} · {tCommon('nights', { count: b.nights })}</p>
